...
import {
  rowIndicatesDailyFreePlayPacing,
...
function isPublicUnmatchedOpenSeatBaseRow(
  row: Pick<
    PublicOpenSeatLobbyRow,
    'black_player_id' | 'status' | 'play_context' | 'tournament_id'
  >,
): boolean {
...
export function isPublicUnmatchedLiveOpenSeatRow(
  ...
  if (!isPublicUnmatchedOpenSeatBaseRow(row)) return false;
  return rowIndicatesLiveFreePlayPacing(row);
}

/** Valid public unmatched daily/async open seat before mode/lane/busy filters. */
export function isPublicUnmatchedDailyOpenSeatRow(
  row: Pick<
    PublicOpenSeatLobbyRow,
    'black_player_id' | 'status' | 'tempo' | 'live_time_control' | 'play_context' | 'tournament_id'
  >,
  ...
  if (!isPublicUnmatchedOpenSeatBaseRow(row)) return false;
  return rowIndicatesDailyFreePlayPacing(row);
}

/** Valid public unmatched open seat (live or daily pacing). */
export function isPublicUnmatchedOpenSeatRow(
  row: Pick<
    PublicOpenSeatLobbyRow,
    'black_player_id' | 'status' | 'tempo' | 'live_time_control' | 'play_context' | 'tournament_id'
  >,
): boolean {
  return isPublicUnmatchedLiveOpenSeatRow(row) || isPublicUnmatchedDailyOpenSeatRow(row);
}
...
>(
  ...
  for (const r of rows) {
    ...
    if (isPublicUnmatchedOpenSeatRow(r)) {
