...
import { assertUserDailyConcurrencyAllowed } from '@/lib/freePlayDailyConcurrency';
...
export async function acceptMatchRequestViaApi(
  ...
  if (normalizeGameTempo(row.tempo) === 'daily' && row.rated === true) {
    const dailyGate = await assertUserDailyConcurrencyAllowed(supabase, authUserId, {
      rated: true,
      action: 'join_or_accept',
    });
    if ('error' in dailyGate) {
      return { ok: false, error: dailyGate.error };
    }
  }
