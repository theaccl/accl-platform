-- PostgREST RPC binds JSON keys to PostgreSQL **parameter names**. Some deployments / schema caches
-- failed to resolve `create_seated_game_guard` when callers used `p_existing_open_seat_id` / `p_row`.
-- Align public names with common PostgREST expectations: `existing_open_seat_id`, `payload`.
-- Logic unchanged from 20260520130000_create_seated_game_guard_match_integrity.sql.
