-- Phase 1F: idempotent move logs + safe duplicate handling in apply_move RPC.

alter table public.game_move_logs
  add column if not exists idempotency_key text;

comment on column public.game_move_logs.idempotency_key is
  'Optional client/server idempotency token; unique per game when set.';
...
comment on function public.apply_move_and_maybe_finish_system(
  ...
  text,
  jsonb
) is 'Atomic move commit with idempotent game_move_logs insert; always returns games row (Phase 1F).';
