-- Widen `match_requests.live_time_control` allowlist: canonical PLAT tokens + common client typos
-- (e.g. 5m+5 vs 5+5) so direct challenges and listings never fail spuriously.
...
comment on constraint match_requests_live_time_control_check on public.match_requests is
  'Allowed clock tokens; keep in sync with lib/freePlayModeTimeControl + lib/gameTimeControl.canonicalLiveTimeControlForInsert.';
