-- 1) Stale live open seats: `tempo` can be null or not exactly 'live' on some rows, so
--    `= 'live'` missed voiding them after another live game activates. Supersede every free open
--    waiting row except daily / correspondence.
-- 2) create_seated_game_guard: do not treat host "busy in a live two-player game" as blocking
--    joining a *daily* or *correspondence* open seat (host may be an active live game).
...
begin
  for r in
    ...
    where play_context = 'free'
      ...
      and lower(btrim(coalesce(tempo, ''))) not in ('daily', 'correspondence')
...
comment on function public.create_seated_game_guard(uuid, jsonb) is
  'Free-play: host_busy skipped for async-paced open seats; supersede voids all non-daily open waits.';
