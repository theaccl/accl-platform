-- Queue rule: players may hold multiple live **open** seats until one becomes a real two-player game.
-- As soon as Black is seated (or a two-player row is inserted), void all **other** stale open seats for
-- both participants, then cancel their pending open **live** match_requests — fast enough that a second
-- live listing cannot be accepted into while already seated elsewhere.
--
-- Changes vs 20260423140000:
-- - `free_play_joiner_busy` only blocks when the joiner is already in another **seated** free game
--   (both colors), not their own solo open seats.
-- - `perform supersede_stale_free_open_seats_for_users` + match_requests cleanup run **after** the
--   open seat is filled (join) or **after** insert (new two-player row), not before.
...
begin
  ...
  if existing_open_seat_id is null then
    ...
    perform public.supersede_stale_free_open_seats_for_users(v_white, v_black, g.id);
    ...
    where mr.status = 'pending'
      ...
      and mr.tempo = 'live'
      and mr.from_user_id in (v_white, v_black);
...
comment on function public.create_seated_game_guard(uuid, jsonb) is
  'Free-play: busy checks (seated games only for joiner), seat black or insert two-player row, then supersede other open seats + cancel live open match_requests.';
