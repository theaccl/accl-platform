...
export function FreeLobbyCurrentGamesPanel({ modeFilter = null, obligations }: Props) {
  ...
  // Dominant surface: seated live board(s) — presence-based, NOT mode-filtered, so a
  // running live game is never hidden by the active mode filter.
  const seatedLiveRecovery = useMemo(
    () => selectSeatedLiveRecoveryRows(freeLiveRaw, uid),
    [freeLiveRaw, uid],
  );

  // Subsection rows are now open/waiting live seats only — seated boards are shown by
  // the sticky banner above to avoid a redundant duplicate row.
