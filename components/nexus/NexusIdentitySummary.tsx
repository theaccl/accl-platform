...
export default function NexusIdentitySummary({ data }: { data: NexusIdentitySummaryData }) {
  return (
    <section
      className={`flex h-full min-h-[11rem] flex-col rounded-2xl border border-[#2a3442] bg-gradient-to-br from-[#111723] to-[#1a2231] p-4 shadow-lg shadow-black/25 ring-1 ring-inset ring-white/[0.04] ${nexusTransition} hover:shadow-xl hover:shadow-black/30 hover:ring-white/[0.07]`}
      aria-label="Identity summary"
    >
      <h2 className={nexusModuleHeadingClass}>Identity</h2>
      {data.isAnonymous ? (
        <div className="flex flex-1 flex-col justify-center rounded-lg border border-dashed border-[#2a3442]/70 bg-[#0a0e14]/50 px-4 py-8 text-center">
