...
A server fixture adapter now generates deterministic PNG bytes through the real local worker, refinement and crop code. Its request provider is `fixture:<scope>`; live and differently scoped workers cannot claim those jobs. Configuration fails closed outside localhost development or the identified disposable Preview database. Fixture processing skips unrelated maintenance, and fixture issue reports fall back to manual review without an AI call. The PostgreSQL WASM flow passed with outbound fetch forbidden and recorded zero provider cost. Hosted Auth/Storage and simultaneous sessions still need approved staging validation. See [implementation evidence](GENERATOR_LAUNCH_IMPLEMENTATION_2026_09_09.md).
