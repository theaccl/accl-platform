...
**Status:** Audit / design + local staging foundation reconciliation (`20260804155321_chess_knowledge_layer_foundation_reconciliation.sql`).
...
Authenticated reads for `finished_game_opening_matches`, `finished_game_tactic_extractions`, `player_repertoire_entries`, and `player_opening_color_stats` are intentionally deferred. RLS policies do not themselves grant table access. Access may be added only with a separately reviewed client read path and RLS verification. Ownership RLS policies may exist; `GRANT SELECT` to `authenticated` on these four tables is not part of this foundation package.
