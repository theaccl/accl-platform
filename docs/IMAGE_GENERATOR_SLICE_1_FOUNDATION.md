...
- Placement preflight verifies owner, candidate moderation, commission approval, and historical single-surface rights before image processing. Completed placements still active on the profile replay their existing derivatives. In-flight distributed publication and cleanup are not yet fully idempotent; see the [September 8 accept-and-place review](image-generator/ACCEPT_AND_PLACE_LOCAL_REVIEW_2026_09_08.md).
- Recovered placement/refinement controls use the commission's recorded tier, and acceptance finishes in-flight browser transitions while preserving the already-loaded private image elements. Community motion stays off even when a generic public-authorization flag is supplied.
