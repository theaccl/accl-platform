...
**Status:** MVP foundation (`20260804155321_chess_knowledge_layer_foundation_reconciliation.sql` + `lib/chessKnowledge/*`).

**Authority:** Documentation and local staging only. This package grants no runtime, operational, or production authority.

---

## Deferred authenticated access (derived tables)

The following derived tables enable ownership-scoped RLS policies for a future client path, but **authenticated reads are intentionally deferred**:

- `finished_game_opening_matches`
- `finished_game_tactic_extractions`
- `player_repertoire_entries`
- `player_opening_color_stats`

Rules:

- RLS policies do **not** themselves grant table access.
- This foundation migration does **not** `GRANT SELECT` on these four tables to `authenticated`.
- Access may be added only with a separately reviewed client read path and RLS verification.
- Reference tables may receive the intended authenticated read grants; writes remain `service_role`-only.
- This package grants no runtime, operational, or production authority.
...
| **Engine artifacts** | Stockfish/tablebase outputs | Chess truth (below tablebase) | `finished_game_analysis_*` (existing); staged offline provenance in `lib/chessKnowledge/engineProvenance.ts` |
...
- Copyrighted/private-reference content remains excluded from imported public data.
- Source-authored, ACCL-generated, and engine-derived metadata remain separate layers.

## Engine-verification provenance

Offline staging reports persist structured provenance sufficient to distinguish:

- engine name;
- engine version (null when not reported — never fabricated);
- depth, when depth-controlled;
- movetime in milliseconds, when time-controlled;
- MultiPV;
- score unit and score/value when available (null otherwise);
- verification status;
- verification timestamp;
- failure or unavailable reason.

A record must not claim engine verification merely because parameters were configured. Use `null` or an explicit unavailable/not-run state when verification did not occur.
