...
test.describe('profile edit — bio word count', () => {
  ...
  test('short optional bio saves successfully', async ({ page }) => {
    ...
    if (await usernameInput.isEnabled()) {
      await usernameInput.fill(`e2e_bio_short_${Date.now()}`);
    }

    await page.getByTestId('edit-profile-bio-input').fill('A concise optional bio.');
  ...
  test('bio over 250 words shows inline error and does not save', async ({ page }) => {
    ...
    await page.getByTestId('edit-profile-save').click();
    await expect(page.getByTestId('edit-profile-bio-error')).toHaveText('Bio must be 250 words or fewer.');
