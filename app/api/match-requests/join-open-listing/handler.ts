...
export async function matchRequestJoinOpenPost(
  request: Request,
  deps: MatchRequestJoinOpenRouteDeps = defaultDeps,
): Promise<Response> {
  ...
  if (rowIndicatesLiveFreePlayPacing(r)) {
    ...
    if (slot) {
      const s = await deps.userInSeatedInSamePlatQueueSlotAdmin(userId, slot);
      if ('queryError' in s) {
  ...
  const gameCreateRes = await serviceSupabase.from('games').insert(challengeRow).select('id').single();
