...
export type MatchRequestDeclineRouteDeps = {
  resolveAuthenticatedUser: typeof resolveAuthenticatedUser;
  bearerToken: typeof bearerToken;
  createServiceRoleClient: typeof createServiceRoleClient;
};

const defaultDeps: MatchRequestDeclineRouteDeps = {
  resolveAuthenticatedUser,
  bearerToken,
  createServiceRoleClient,
};

export async function matchRequestDeclinePost(
  request: Request,
  deps: MatchRequestDeclineRouteDeps = defaultDeps,
): Promise<Response> {
  ...
  const { data: row, error: fetchErr } = await serviceSupabase
    ...
    .maybeSingle();
  if (fetchErr) {
    console.warn('[match-requests.decline] fetch failed', fetchErr.message);
  ...
  if (String(row.to_user_id ?? '') !== user.id) return jsonResponse({ error: 'Forbidden' }, 403);
  ...
  if (uErr) {
    console.warn('[match-requests.decline] update failed', uErr.message);
