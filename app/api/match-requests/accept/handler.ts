...
export async function matchRequestAcceptPost(
  request: Request,
  deps: MatchRequestAcceptRouteDeps = defaultDeps,
): Promise<Response> {
  ...
  if (isDirectOrPrivateLivePacedMatchRequest(r)) {
    ...
    if (slot) {
      const c = await deps.userHasConflictingPlatQueueSlotAdmin(userId, slot);
